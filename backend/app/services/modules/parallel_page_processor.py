...
class ParallelPageProcessor:
    ...
    def __init__(self, pdf_processor: PDFProcessor, llm_client: LLMClient, prompt_service: PromptService, yolo_detector: Optional[YOLOSignatureDetector] = None, face_detector: Optional[YOLOFaceDetector] = None):
        ...
        # Use provided YOLO detectors or create new ones (for backward compatibility)
    ...
    async def process_pages_parallel(
        ...
        logger.debug(f"🚀 Starting multi-stage pipeline processing of {total_pages} pages with {effective_max_workers} workers per stage")
        ...
        try:
            ...
            page_data: Dict[int, Dict[str, Any]] = {i: {"page_num": i} for i in range(total_pages)}
            ...
            if prefer_text:
                ...
                logger.info(f"⏭️ Skipping text extraction stages for all {total_pages} page(s)")
            ...
            while completion_counts[9] < total_pages:
            ...
            logger.debug(f"✅ All pages completed: {completion_counts[9]}/{total_pages} pages finished Stage 9")
            ...
            error_count = total_pages - success_count
            logger.debug(f"✅ Pipeline complete: {success_count} successful, {error_count} errors")
